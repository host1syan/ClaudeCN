# ClaudeCN 多 Agent 系统 — 使用指南

> 让 ClaudeCN 同时调度多个专业代理，并行处理复杂任务。
...
ClaudeCN 的多 Agent 系统是一套**智能任务编排框架**，让主代理能够生成多个专业化的子代理（Subagent），各自独立执行不同的任务，最终将结果汇总给用户。
...
ClaudeCN 内置了 6 种专业代理，每种都有特定的工具池和适用场景：
...
**适用场景**：回答关于 ClaudeCN、Agent SDK、Claude API 的问题。
...
当多个来源定义了同名 Agent 时，ClaudeCN 按以下优先级选择实际生效的定义（从高到低）：
