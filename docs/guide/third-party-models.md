...
- **Kilo Free**：通过 Kilo Gateway 使用免费模型，默认模型为 `stepfun/step-3.7-flash:free`。

免费渠道依赖上游服务。模型目录、额度、限流、响应速度和可用性可能变化，项目不承诺永久免费、无限额度或固定可用率。桌面端可在设置中选择渠道和模型；也可继续使用下方的自定义 API、LiteLLM 或其他协议转换代理。

项目支持 Anthropic Prompt Cache 相关字段和缓存用量映射。长任务中，稳定系统提示词、工具列表和请求前缀更容易复用缓存；实际命中取决于上游模型、缓存 TTL、请求结构和上下文变化。使用第三方模型或协议转换代理时，代理可能丢弃 `cache_control`，不能默认认为缓存生效。
