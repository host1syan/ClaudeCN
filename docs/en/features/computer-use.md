...
> **Modified Version**: This feature is a **heavily modified version** of the Computer Use (internal codename "Chicago") found in the Claude Code source. The official implementation relies on Anthropic's private native modules (`@ant/computer-use-swift`, `@ant/computer-use-input`) that are not publicly available. We **replaced the entire underlying operation layer** with a Python bridge: macOS uses `pyautogui` + `mss` + `pyobjc`, and Windows uses `pyautogui` + `mss` + `win32gui` + `psutil`.
...
```bash
claude-cn --no-computer-use
CLAUDE_COMPUTER_USE_ENABLED=0 claude-cn
```

You can also write the global config file at `~/.claude/claude-cn/computer-use-config.json`:
...
| [oboard/claude-code-rev](https://github.com/oboard/claude-code-rev) | - | Early original-source restoration, stub package reference |
